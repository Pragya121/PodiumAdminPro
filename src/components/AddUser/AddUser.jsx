...
const postUser = async (finalData) => {
  try {
    ...
    const response = await axios.post(`${BASE_URL}/internalUsers`,finalData,config);
...
return;
    ...
    if(formData.data.writerSkills ||formData.data.writerDomains && roleType ==="writer" ){
      alert("please enter domains and skills");
      return; 
          }
          if(formData.data.writerSkills ||formData.data.writerDomains && roleType ==="editor" ){
            alert("please enter domains and skills");
            return; 
                }
    let finalData = {
...
onChange={ (newValue, actionMeta) => {
  ...
  options={field.fname==="writerDomains"? dFields.writerDomains:dFields.writerSkills} />
</div>

                      <br/></>)
                       else  if(field.key ===5 && field.options.length !==0 && field.fname ==="commercialAgreement")
                       return(
                       <>  
